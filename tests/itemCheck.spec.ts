...
import { listView, lookForSection, login, timeoutValue } from './supportFunctions.spec';
...
export async function checkItemFunc(page: any, itemId: string, unitVolume: string, section: string): Promise<void> {
    try {
        ...
        await iframe.getByTitle('Open record "' + itemId + '"').waitFor({ timeout: timeoutValue }).catch(() => {
